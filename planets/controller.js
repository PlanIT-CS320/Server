		...
		if (planet) {
			...
			if (planetUserMatch || req.user.role == "admin") {
				...
				for (let i = 0; i < allMatches.length; i++) {
					...
					const { password, createdAt, updatedAt, role, ...userInfo } =
						user._doc; //Omit fields from user
	...
	try {
		...
		if (!name || !description || !ownerId) {
			return res.status(400).json({
				message:
					"Missing information: Cannot create planet without name, description, and ownerId.",
			});
		}
			...
			if (owner._id == req.user.userId || req.user.role == "admin") {
				...
				try {
					...
					return res.status(500).json({
						message:
							"Internal server error. Contact support or try again later.",
					});
				}
...
async function getColumns(req, res) {
	try {
		...
		if (planet) {
			...
			const planetUserMatch = PlanetCollaborator.exists({
				planetId,
				userId: req.user.userId,
			});
			if (planetUserMatch || req.user.role == "admin") {
			...
			return res.status(403).json({
				message:
					"You do not have permission to access the columns of this planet.",
			});
		}
...
async function createTask(req, res) {
	try {
		...
		if (!content) {
			...
			return res.status(400).json({
				message:
					"Missing information. Cannot create task without task content.",
			});
		}
		...
		if (column) {
			...
			return res.status(403).json({
				message:
					"User does not have permission to create a task in this planet.",
			});
		}
		//Column not found
		return res.status(404).json({
...
async function createColumn(req, res) {
	try {
		...
		if (planet) {
			...
			return res.status(403).json({
				message:
					"You do not have permission to create a column in this planet.",
			});
		}
		//Planet not found
		return res.status(404).json({
	...
	try {
		...
		if (planet) {
			...
			if (planetInviterMatch) {
				...
				return res.status(403).json({
					message:
						"Only the planet owner has permission to send invites for this planet.",
				});
			}
			//Id of token is not a member of planet
			return res.status(403).json({
...
async function updateTask(req, res) {
	try {
		...
		const filteredFields = passedFields.filter((field) =>
							...
							if (req.body.assignedUserId == null) {
								...
								if (assignedUser) {
									...
									if (planetUserMatch) {
										...
										return res.status(400).json({
											message:
												"New assigned user does not have permission to work on this task.",
										});
									}
								} else {
									return res.status(404).json({
...
async function deleteColumn(req, res) {
	try {
		...
		if (column) {
			...
			console.log(
				"Orphaned data of type planet_column found: " + orphanedColumns,
			);
			return res.status(404).json({
		...
		if (planet) {
			...
			if (user) {
				...
				if (planetUserMatch) {
					...
					if (
						planetRequesterMatch?.role == "owner" ||
						req.user.role == "admin"
					) {
						//Remove user
					...
					return res.status(403).json({
						message:
							"You do not have permission to remove users from this planet.",
					});
				}
		...
		if (planet) {
			...
			if (user) {
				...
				if (planetUserMatch) {
					...
					return res.status(403).json({
						message:
							"You do not have permission to promote users in this planet.",
					});
				}
				//User not a member of planet
				return res.status(404).json({
